...
const CategoryPage = () => {
  ...
  // Map category slug to display names
  const categoryMap = {
    'food': 'غذا و نوشیدنی',
  ...
  const filteredDiscounts = categoryMap[categorySlug]
    ? discounts.filter(discount => 
        discount.categories && discount.categories.includes(categoryMap[categorySlug])
      )
    : [];
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: 'IRANYekan' }}>
      ...
      <div className="w-full px-4 md:px-[120px] pt-3 md:pt-[12px] mb-4 md:mb-6">
        <nav className="flex items-center gap-2 text-xs md:text-sm text-gray-500">
          ...
          <span className="text-orange-500 font-medium">{categoryName}</span>
        </nav>
      </div>
